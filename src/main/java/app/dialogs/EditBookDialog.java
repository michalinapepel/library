...
public class EditBookDialog extends JDialog implements LanguageChangeListener {
	...
	public EditBookDialog(JFrame parent, Book book, Author[] authors, Shelf[] shelves, Bookcase[] bookcases,
			Section[] sections) {
	...
	private void selectAuthorById(int authorId) {
		for (int i = 0; i < authorField.getItemCount(); i++) {
			Author author = authorField.getItemAt(i);

			if (author != null && author.getId() == authorId) {
				authorField.setSelectedIndex(i);
				return;
			}
		}

		authorField.setSelectedIndex(-1);
	}
	...
	private void initComponents() {
		...
		ok.addActionListener(e -> {
			...
			if (title.isEmpty() || publisher.isEmpty() || isbn.isEmpty()) {
				JOptionPane.showMessageDialog(this, Localization.get("validation.all.required"),
						Localization.get("message.validation.error"), JOptionPane.WARNING_MESSAGE);
				return;
			}
			if (!isbn.matches("\\d{10}|\\d{13}")) {
				JOptionPane.showMessageDialog(this, Localization.get("validation.isbn.format"),
						Localization.get("message.validation.error"), JOptionPane.WARNING_MESSAGE);
				return;
			}
			...
			if (selectedAuthor == null) {
				int confirm = JOptionPane.showConfirmDialog(this, Localization.get("message.no.author.edit"),
						Localization.get("dialog.no.author.title"), JOptionPane.YES_NO_OPTION,
	...
	private void loadBookData() {
		...
		if (originalBook.getId() > 0) {
			...
			if (!bookAuthors.isEmpty()) {
				selectAuthorById(bookAuthors.get(0).getId());
			} else {
				authorField.setSelectedIndex(-1);
			}
